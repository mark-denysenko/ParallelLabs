...
public class ProcessGenerator extends Thread {
    ...
    public void run() {
        ...
        for (int i = 0; i < this.n_processes; i++) {
            int executionTimeMillis = (random.nextInt(this.maxProcessTime - this.minProcessTime + 1) + this.minProcessTime);
            var process = new CPUProcess(this.processKind, executionTimeMillis);
